...
class UsfaHead(hk.Module):
  """Page 17."""
  def __init__(self,
    task_bank: jnp.ndarray, #all the possible tasks we could have
    num_actions: int,
    state_dim: int,
    hidden_size : int=128,
    policy_size : int=32,
    ...
    self.nsamples = task_bank.shape[0]
    self.num_actions = num_actions
    ...
    self.all_w = jax.vmap(self.task_embed)(self.task_bank)
    if self.normalize_task:
      self.all_w = self.all_w/(1e-5+jnp.linalg.norm(self.all_w, axis=-1, keepdims=True))
  ...
  def __call__(self,
    ...
    #TODO: replace this sampling with just using all the W's for different tasks DONE
    z_samples = self.all_w
