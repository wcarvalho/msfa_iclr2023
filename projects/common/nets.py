...
def make_farm_prep_fn(num_actions, task_input=True, embed_task=None):
  """
...
def build_task_embedder(task_embedding, config):
...
def r2d1_prediction_prep_fn(inputs, memory_out,
  task_embedder = lambda x: x,
  **kwargs):
  """
...
def r2d1(config, env_spec,
  ...
  if task_input:
    prediction_prep_fn = functools.partial(r2d1_prediction_prep_fn,
      task_embedder=build_task_embedder(task_embedding, config)
      )
  else:
