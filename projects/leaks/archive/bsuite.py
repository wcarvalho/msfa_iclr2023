...
flags.DEFINE_integer('num_episodes', int(100e6), 'Number of episodes to train for.')
flags.DEFINE_integer('seed', 0, 'Random seed.')

# -----------------------
# for finding memory leak
# -----------------------
flags.DEFINE_boolean('distributed', True, 'Whether to do distributed')
...
class DistributedTDAgent(distributed_layout.DistributedLayout):
  ...
  def __init__(
    ...
    policy_network_factory = (
        lambda n: r2d2.make_behavior_policy(n, config))
    evaluator_policy_network_factory = (
        lambda n: r2d2.make_behavior_policy(n, config, True))
...
def distributed():
  environment_factory = lambda is_eval: make_environment()
