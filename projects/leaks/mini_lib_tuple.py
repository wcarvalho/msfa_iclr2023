...
from typing import Callable, Iterator, List, Optional, Tuple, NamedTuple
...
class R2D2Learning(learning_lib.LossFn):
  ...
  def __call__(
    ...
    batch_td_error, batch_loss = self.error(data, online_q.q, online_state, target_q.q, target_state)
...
class DistributedTDAgent(distributed_layout.DistributedLayout):
  ...
  def __init__(
      ...
      builder=TDBuilder,
      behavior_policy_constructor=r2d2.make_behavior_policy,
      # workdir: Optional[str] = '~/acme',
...
def r2d1(config, env_spec):
  ...
  return SimpleRecurrentQNetwork(num_actions=num_actions)
...
def make_networks(batch_size, env_spec, NetworkCls, NetKwargs):
  ...
  def unroll_fn(inputs, state, k):
    model = NetworkCls(**NetKwargs)
    return model.unroll(inputs, state, k)
  ...
  def unroll_init_fn(rng, initial_state):
    return unroll_hk.init(rng, dummy_obs_sequence, initial_state, rng)
