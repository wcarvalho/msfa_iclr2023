...
class R2D2Learner(acme.Learner):
  ...
  def __init__(
      ...
      # target_network: Union[networks.RNNCore, hk.RNNCore],
      burn_in_length: int,
      sequence_length: int,
    ...
    if not isinstance(network, networks.RNNCore):
      network.unroll = functools.partial(hk.static_unroll, network)
      target_network.unroll = functools.partial(hk.static_unroll,
                                                target_network)

    # Internalise agent components (replay buffer, networks, optimizer).
    # TODO(b/155086959): Fix type stubs and remove.
    self._iterator = utils.prefetch(data_iterator)
    # self._iterator: Iterator[reverb.ReplaySample] = iter(dataset)  # pytype: disable=wrong-arg-types
    self._network = network
    self._target_network = target_network
    self._optimizer = hk.optimizers.Adam(learning_rate, epsilon=1e-3)
  ...
  def _step(self) -> Dict[str, tf.Tensor]:
    ...
    data = jax_utils.batch_to_sequence(sample.data)
  ...
  def get_variables(self, names: List[str]) -> List[Variables]:
    return [jax_utils.to_numpy(self._variables)]

  @property
  def state(self) -> Mapping[str, jax_savers.Checkpointable]:
